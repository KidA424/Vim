...
syn match sasComment	"^\s*%*\*.*;" contains=sasTodo
syn match sasODS	"^ods.*;" contains=sasTodo
syn match sasOPT	"^options.*;" contains=sasTodo
...
syn keyword sasStatement	MERGE MISSING MODIFY OUTPUT PAGE
...
if version >= 508 || !exists("did_sas_syntax_inits")
   ...
   " Default sas enhanced editor color syntax
	hi sComment	term=bold cterm=NONE ctermfg=Green ctermbg=Black guifg=Red guibg=Black
...
" these override the next section, which seems to do nothing
" use :hi to see what colors to pick
   HiLink sasConditional	Statement
   HiLink sasStep		Statement
   HiLink sasFunction		Function
   HiLink sasMacro		Statement
   HiLink sasMacroVar		Special
   HiLink sasNumber		Number
   HiLink sasStatement		MoreMSG
   HiLink sasString		Function
   HiLink sasErrMsg		ErrorMsg
   HiLink sasWarnMsg		WarningMsg
   HiLink sasLogMsg		MoreMsg
   HiLink sasCards		MoreMsg
   HiLink sasComment	Question
   HiLink sasODS	Comment 
   HiLink sasOPTIONS	Special 
 
             
" the below are overriden by the previous section
